...
public:
  BaseOOPETest()
    : map{std::make_shared<MockGridCell>(Default_Prob), {100, 100, Map_Scale}} {
...
class ObstacleBasedOOPETest
    : public BaseOOPETest<ObstacleBasedOccupancyObservationPE> {};
...
TEST_F(ObstacleBasedOOPETest, areaNeutrality) {
  test_oope(1, mid<0,0>(), {-1000, 1000, -1000, 1000});
}

TEST_F(ObstacleBasedOOPETest, shiftInsideCellNeutrality) {
  test_oope(1, mid<0,0>());
  test_oope(1, mid<0,0>() + Shift{Map_Scale, Map_Scale} * 0.25);
}

TEST_F(ObstacleBasedOOPETest, shiftNearCell) {
  test_oope(1.00, mid<0,0>());
  test_oope(0.25, mid<0,0>() + Shift{0, Map_Scale});
  test_oope(0.50, mid<0,0>() + Shift{Map_Scale, 0});
  test_oope(0.00, mid<0,0>() + Shift{Map_Scale, Map_Scale});
}

/******************************************************************************/
// == MaxOOPE tests

class MaxOOPETest : public BaseOOPETest<MaxOccupancyObservationPE> {};
...
TEST_F(MaxOOPETest, shiftInsideCell) {
  ...
  test_oope(0.25, mid<0,1>(), range);
  test_oope(1, mid<0,1>() + Shift{0, -Map_Scale} * 0.5, range);
  test_oope(0.25, mid<0,1>() + Shift{Map_Scale, 0} * 0.5, range);
  test_oope(1, mid<0,1>() + Shift{Map_Scale, -Map_Scale} * 0.5, range);
  test_oope(0.5, mid<1,0>() + Shift{0, Map_Scale} * 0.5, range);
}

TEST_F(MaxOOPETest, severalCellCoverNeutralityToSmallShift) {
  auto range = cell_range().shrink(2);
  test_oope(0.25, mid<0,1>(), range);
  test_oope(1, mid<0,1>() + Shift{0, -Map_Scale} * 0.375, range);
  test_oope(0.25, mid<0,1>() + Shift{Map_Scale, 0} * 0.375, range);
  test_oope(1, mid<0,1>() + Shift{Map_Scale, -Map_Scale} * 0.375, range);
}

TEST_F(MaxOOPETest, observationAreaScaling) {
  auto range = cell_range();
  test_oope(Default_Prob, mid<1, -1>(), range.shrink(2));
  test_oope(0.5, mid<1, -1>(), range);
  test_oope(1, mid<1,-1>(), range.shrink(0.5));
}

/******************************************************************************/
// == MeanOOPE tests

class MeanOOPETest : public BaseOOPETest<MeanOccupancyObservationPE> {};
...
TEST_F(MeanOOPETest, shiftInsideCell) {
  auto range = cell_range().shrink(2);
  test_oope(0.25, mid<0,1>(), range);
  test_oope(1.25 / 2, mid<0,1>() + Shift{0, -Map_Scale} * 0.5, range);
  test_oope(0.25 / 2, mid<0,1>() + Shift{Map_Scale, 0} * 0.5, range);
  test_oope(1.75 / 4, mid<0,1>() + Shift{Map_Scale, -Map_Scale} * 0.5, range);
}
...
TEST_F(MeanOOPETest, severalCellCoverNeutralityToSmallShift) {
  ...
  test_oope(1.25 / 2, mid<0,1>() + Shift{0, -Map_Scale} * 0.375, range);
  test_oope(0.25 / 2, mid<0,1>() + Shift{Map_Scale, 0} * 0.375, range);
  test_oope(1.75 / 4, mid<0,1>() + Shift{Map_Scale, -Map_Scale} * 0.375, range);
  test_oope(0.5 / 2, mid<1,0>() + Shift{0, Map_Scale} * 0.375, range);
}

TEST_F(MeanOOPETest, observationAreaScaling) {
  auto range = cell_range();
  test_oope(Default_Prob, mid<1, -1>(), range.shrink(2));
  test_oope(0.5 / 4, mid<1, -1>(), range);
  test_oope(1.5 / 9, mid<1,-1>(), range.shrink(0.5));
}
...
class OverlapWeghtedOOPETest
    : public BaseOOPETest<OverlapWeightedOccupancyObservationPE> {};
...
TEST_F(OverlapWeghtedOOPETest, shiftInsideCell) {
  auto range = cell_range().shrink(2);
  test_oope(0.25, mid<0,1>(), range);
  test_oope(0.625, mid<0,1>() + Shift{0, -Map_Scale} * 0.5, range);
  test_oope(0.25 / 2, mid<0,1>() + Shift{Map_Scale, 0} * 0.5, range);
  test_oope(1.75 / 4, mid<0,1>() + Shift{Map_Scale, -Map_Scale} * 0.5, range);
  test_oope(0.5 / 2, mid<1,0>() + Shift{0, Map_Scale} * 0.5, range);
}

TEST_F(OverlapWeghtedOOPETest, severalCellCoverSmallShift) {
  auto range = cell_range().shrink(2);
  test_oope(0.25, mid<0,1>(), range);
  test_oope(0.4375, mid<0,1>() + Shift{0, -Map_Scale} * 0.375, range);
  test_oope(0.1875, mid<0,1>() + Shift{Map_Scale, 0} * 0.375, range);
  test_oope(0.25*0.25*0.5 + 0.25 * 0.75 * 1 + 0.75 * 0.75 * 0.25,
            mid<0,1>() + Shift{Map_Scale, -Map_Scale} * 0.375, range);
}

TEST_F(OverlapWeghtedOOPETest, observationAreaScaling) {
  auto range = cell_range();
  test_oope(0.5*0.25 + 1*0.125 + 0.25*0.0625, mid<1,0>(), range.shrink(0.5));
}

TEST_F(OverlapWeghtedOOPETest, observationAreaScalingSmalShift) {
  auto range = cell_range();
  test_oope((0.5*1 + 1*0.25 + 0.25*0.0625) / 2.25,
