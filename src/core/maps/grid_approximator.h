...
public:
  ...
  virtual unsigned max_approximation_level() const = 0;
  virtual const GridMap& map(unsigned apprx_lvl) const = 0;
...
public:
  ...
  unsigned max_approximation_level() const override {
    return _approximated_maps.size() - 1;
  }

  const GridMap& map(unsigned lvl) const override {
    assert(0 <= lvl && lvl <= max_approximation_level());
  ...
  void on_grid_map_update(const GridMap& updated_map,
    ...
    for (unsigned al = 0; al <= max_approximation_level(); ++al) {
      if (unhandled_pts.empty()) { break; }

      auto &coarse_map = modifiable_map(al);
    ...
    ensure_approximation_cache_continuous();
  };
...
private:
  ...
  GridMap& modifiable_map(unsigned lvl) {
    auto const_this = static_cast<const decltype(this)>(this);
    return const_cast<GridMap&>(const_this->map(lvl));
  }
  ...
  void ensure_approximation_cache_continuous() const {
    ...
    auto &pre_coarsest_map = map(max_approximation_level() - 1);
